(ns wallet.base58-test
  (:require [clojure.test :refer :all]
            [wallet.base58 :refer :all]
            [clojure.string :as str]))

(defonce simple-test-data [[""
                            ""
                            "3QJmnh"]
                           ["hello"
                            "Cn8eVZg"
                            "2L5B5yqsVG8Vt"]
                           ["The quick brown fox jumps over the lazy dog"
                            "7DdiPPYtxLjCD3wA1po2rvZHTDYjkZYiEtazrfiwJcwnKCizhGFhBGHeRdx"
                            "hgrbmYjTAB9gMSwZ6bUP86rvvhPkJzRkcqkmdZXXPyQCbXzuSLGmEDgmK4iSfhDR"]])

(defn- encode-str
  ([s]
   (encode-str s false))
  ([s checksum?]
   (->> s
        (.getBytes)
        ((if checksum?
           encode-check
           encode)))))

(deftest simple-cases
  (run! (fn [[in-str expect1 expect2]]
          (testing (str "Simple case: " in-str)
            (is (= expect1 (encode-str in-str)))
            (is (= expect2 (encode-str in-str true)))
            (is (= in-str (-> in-str encode-str decode)))
            (is (= in-str (-> in-str (encode-str true) decode-check)))))
        simple-test-data))

(deftest leading-zero-cases
  (testing "Leading zeros"
    (is (= "11ZiCa" (encode-str "\000\000abc")))
    (is (= "\000\000abc" (-> "\000\000abc" encode-str decode)))
    (is (= "114h3c8kNw2W" (encode-str "\000\000abc" true)))
    (is (= "\000\000abc" (-> "\000\000abc" (encode-str true) decode-check)))
    (is (= "1111" (encode-str "\000\000\000\000")))
    (is (= "\000\000\000\000" (-> "\000\000\000\000" encode-str decode)))
    (is (= "11114bdQda" (encode-str "\000\000\000\000" true)))
    (is (= "\000\000\000\000" (-> "\000\000\000\000" (encode-str true) decode-check)))))

(deftest boundary-values
  (testing "Boundary values"
    (is (= "5Q" (-> [0xff] byte-array encode)))
    (is (= [0xff] (->> [0xff] byte-array encode decode (mapv int))))
    (is (= "VrZDWwe" (-> [0xff] byte-array encode-check)))
    (is (= [0xff] (->> [0xff] byte-array encode-check decode-check (mapv int))))))

(deftest alternating-bytes
  (testing "Alternating bytes"
    (is (= "7X4BGT" (->> [0xff 0 0xff 0] byte-array encode)))
    (is (= [0xff 0 0xff 0] (->> [0xff 0 0xff 0] byte-array encode decode (mapv int))))
    (is (= "jeriXwYhkjo" (->> [0xff 0 0xff 0] byte-array encode-check)))
    (is (= [0xff 0 0xff 0] (->> [0xff 0 0xff 0] byte-array encode-check decode-check (mapv int))))))

(deftest minimal-input
  (testing "Minimal input"
    (is (= "1" (->> [0] byte-array encode)))
    (is (= [0] (->> [0] byte-array encode decode (mapv int))))
    (is (= "1Wh4bh" (->> [0] byte-array encode-check)))
    (is (= [0] (->> [0] byte-array encode-check decode-check (mapv int))))
    (is (= "2g" (encode-str "a")))
    (is (= "a" (->> (encode-str "a") decode)))
    (is (= "a" (->> (encode-str "a" true) decode-check)))))

(deftest long-input
  (testing "Long input"
    (is (= "" (->> \a (repeat 1000) (apply str) encode-str)))
    (is (= ""  (encode-str (->> \a (repeat 1000) (apply str)) true)))))
